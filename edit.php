...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $imageName = $item['image']; // เก็บค่าเดิม
    $pdfName = $item['pdf_file']; // เก็บค่าเดิม
...
$imageExists = !empty($item['image']) && file_exists($uploadDir . $item['image']);
$pdfExists = !empty($item['pdf_file']) && file_exists($uploadDir . $item['pdf_file']);
?>
...
<body>
    ...
    <div class="container mt-5">
        <h1 class="mb-4"><i class="fas fa-edit me-2"></i>แก้ไขข้อมูล</h1>

        <?php if ($errors): ?>
        <div class="alert alert-danger alert-dismissible fade show" role="alert">
            <ul class="mb-0">
                <?php foreach ($errors as $error): ?>
                    <li><i class="fas Palae:1px solid #dc3545;"><?php echo htmlspecialchars($error); ?></li>
                <?php endforeach; ?>
            </ul>
        ...
        <form action="edit.php?id=<?= $id ?>" method="post" enctype="multipart/form-data">
            <div class="mb-3">
                ...
                <input type="text" name="name" id="name" class="form-control" value="<?php echo htmlspecialchars($item['name']); ?>" required aria-describedby="nameHelp">
                <div id="nameHelp" class="form-text">กรุณากรอกชื่อที่ต้องการแก้ไข</div>
            </div>
            <div class="mb-3">
                <label for="details" class="form-label"><i class="fas fa-info-circle me-2"></i>รายละเอียด</label>
                <textarea name="details" id="details" class="form-control" rows="4" aria-describedby="detailsHelp"><?php echo htmlspecialchars($item['details']); ?></textarea>
                <div id="detailsHelp" class="form-text">กรุณากรอกรายละเอียดเพิ่มเติม</div>
            </div>
            <div class="mb-3">
                <label for="image" class="form-label"><i class="fas fa-image me-2"></i>รูปภาพ</label>
                ...
                <?php if ($imageExists): ?>
                    <img src="./uploads/<?php echo htmlspecialchars($item['image']); ?>" alt="ภาพปัจจุบัน" class="img-thumbnail mt-2" width="150">
                <?php else: ?>
                    <p class="mt-2 text-warning">ไม่พบรูปภาพปัจจุบัน</p>
                <?php endif; ?>
            </div>
            <div class="mb-3">
                <label for="pdf_file" class="form-label"><i class="fas fa-file-pdf me-2"></i>ไฟล์ PDF</label>
                ...
                <?php if ($pdfExists): ?>
                    <p class="mt-2">
                        <a href="./uploads/<?php echo htmlspecialchars($item['pdf_file']); ?>" target="_blank" class="current-file-link">ดูไฟล์ปัจจุบัน</a>
                    </p>
                <?php else: ?>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            document.addEventListener("keydown", function (event) {
                ...
                if (e.target.files[0] && e.target.files[0].size > maxSize) {
                    alert('ขนาดไฟล์ PDF ต้องไม่เกิน 100MB');
                    e.target.value = '';
                }
            });
                        // Inactivity Timeout
